...
Бот автоматически:
1. **Определяет слабость** - находит активы, которые слабее BTC (**RS < -3%** обязательно)
...
5. **Фильтрует качество** - отправляет только сигналы со скором **≥ 50** (средняя+ вероятность)

## 🚦 Требования к сигналам

**Обязательные условия** (иначе сигнал отбрасывается):
- ✅ **RS < -3%** - актив должен быть слабее BTC минимум на 3%
- ✅ **Close < Support** - цена закрытия пробила уровень поддержки
- ✅ **Volume > 1.5x** - объём выше среднего в 1.5+ раза
- ✅ **Score ≥ 50** - итоговый балл минимум 50 (средняя вероятность)
...
**Только сигналы с Score ≥ 50 отправляются в Telegram!**
